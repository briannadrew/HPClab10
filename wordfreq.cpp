...
void fileread(int itask, char *fname, KeyValue *kv, void *ptr)
{
  ...
  while (word) {
    ...
    int c = 0;
    char strCopy[length]; // copy of word WITHOUT vowels
    for(int i = 0; i < length; i++) {
      // if current character in word is not a value, it can be copied
    ...
    strCopy[c] = '\0';
    strcpy(word, strCopy); // replace word with vowel-less version
...
int ncompare(char *p1, int len1, char *p2, int len2)
{
  int i1 = *(int *) p1;
  int i2 = *(int *) p2;
  // if (i1 > i2) return -1; // original
  if (i1 < i2) return -1;
  // else if (i1 < i2) return 1; // original
